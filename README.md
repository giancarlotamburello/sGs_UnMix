...
The user must draw a perimeter on the map to enclose the data for processing. This can be done by drawing a rectangle or a polygon. Alternatively, a .csv file with the vertices of a polygon can be uploaded by pressing `Upload Polygon`. Download the file [cf_July2000_polygon.csv](https://github.com/giancarlotamburello/sGs_UnMix/blob/main/data_examples/cf_July2000_polygon.csv) as example. The `Remove Polygon` button can remove the uploaded polygon to define a new perimeter.
